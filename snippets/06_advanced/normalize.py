...
def normalize(df, some_numeric_col_list: list[str] | None, normalization_option: NormalizationOptions, result: list[str] | None):
    ...
    :param some_numeric_col_list: Names of the numeric columns to normalize
    ...
    :param result: Column names to write normalization results (Optional)
    ...
    for i, col in enumerate(some_numeric_col_list):
        normalized_col_name = result[i] if result is not None and len(result) == len(some_numeric_col_list) else f"{col}_normalized"
